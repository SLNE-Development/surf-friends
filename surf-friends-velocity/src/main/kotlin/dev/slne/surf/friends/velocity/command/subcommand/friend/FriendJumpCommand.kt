        ...
        playerExecutor { player, args ->
            container.launch {
                ...
                if(targetUuid.isOnline()) {
                    val onlineFriend = plugin.proxy.getPlayer(targetUuid).getOrNull() ?: return@launch run {
                        player.uniqueId.sendText {
                            error("Der Spieler $target ist nicht online.")
                        }
                    }

                    player.createConnectionRequest(onlineFriend.currentServer.getOrNull()?.server ?: return@launch run {
                        player.uniqueId.sendText {
